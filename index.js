  ...
  dataForSunburst.forEach(sunweek => {
    dataByWeek.forEach(week =>{
      if( week.week === sunweek.week && week.year === sunweek.year){
        week.datasets.forEach(label =>{
          if(sunweek.children.filter(child => child.name === fatherReference(label.label,referenceTypes)).length === 0){
...
// document.write("<br>")
 let dayweek = new Date(day.date.split("/")[2],day.date.split("/")[1]-1,day.date.split("/")[0]).getDay();
...
function dateToWeek(date){
  let newDate = new Date(dateToYear(date),date.split("/")[1]-1,date.split("/")[0]);
...
function showData(data){
  if(data.length > 0){ //Si data es array
  data.forEach(item =>{
...
dataForSunburst = generateDataForSunburst(generateDataForBars(data))[weekValue]
...
x = d3.scaleLinear() // continuous scale. preserves proportional differences
    .range([0, 2 * Math.PI]); // setting range from 0 to 2 * circumference of a circle

y = d3.scaleSqrt() // continuous power scale 
...
// define tooltip
tooltip = d3.select('body') // select element in the DOM with id 'chart'
...
legendContainer = d3.select("#legend").append("div").classed("legends clearfix", true);
...
let dataForSunburst = generateDataForSunburst(generateDataForBars(data))[currentWeek]
